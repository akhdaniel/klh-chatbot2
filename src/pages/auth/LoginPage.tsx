...
export default function LoginPage({ onSuccess }: { onSuccess?: () => void }) {
  ...
  if (showSignup) {
    return <SignupPage onBackToLogin={() => setShowSignup(false)} onSuccess={onSuccess} />
  }
    ...
    <div style={{ minHeight: '100vh', display: 'flex', alignItems: 'center', justifyContent: 'center', background: 'var(--paper)', padding: '20px' }}>
      <div style={{ maxWidth: 400, width: '100%', background: 'white', border: '1.5px solid var(--ink)', padding: '40px 32px', boxShadow: '0 20px 50px rgba(13,59,46,0.12)' }}>
        ...
        <div style={{ marginBottom: 32, textAlign: 'center' }}>
          ...
          <h1 style={{ fontFamily: 'Fraunces, serif', fontSize: 28, fontWeight: 500, color: 'var(--leaf-deep)', letterSpacing: '-0.01em' }}>Login</h1>
          <p style={{ fontSize: 13, color: 'var(--bark-soft)', marginTop: 6 }}>Dashboard Pengelola Chatbot KLH</p>
        </div>
        ...
        <div style={{ textAlign: 'center', marginTop: 20, paddingTop: 20, borderTop: '1px solid var(--line)' }}>
          <p style={{ fontSize: 13, color: 'var(--bark-soft)', marginBottom: 8 }}>Belum punya akun?</p>
          <button
    ...
    <div style={{ minHeight: '100vh', display: 'flex', alignItems: 'center', justifyContent: 'center', background: 'var(--paper)', padding: '20px' }}>
      <div style={{ maxWidth: 400, width: '100%', background: 'white', border: '1.5px solid var(--ink)', padding: '40px 32px', boxShadow: '0 20px 50px rgba(13,59,46,0.12)' }}>
        ...
        <form onSubmit={handleSignup} style={{ display: 'flex', flexDirection: 'column', gap: 16 }}>
          ...
          <button
            type="submit"
            disabled={loading || !handleValidation()}
            style={{
              padding: '12px 16px',
              fontSize: 13,
              fontWeight: 600,
