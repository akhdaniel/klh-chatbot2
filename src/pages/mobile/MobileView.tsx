...
export default function MobileView() {
  ...
  const Annotations = () => (
    <div style={{ display: 'flex', flexDirection: 'column', gap: 20, paddingTop: isMobile ? 0 : 40 }}>
      {/* Scenario toggle */}
      <div style={{ display: 'flex', gap: 6, border: '1px solid var(--ink)', padding: 3, background: 'var(--paper)' }}>
        {SCENARIOS.map(s => (
          <button key={s.id} onClick={() => setScenario(s.id)} style={{
            ...
            border: 'none', padding: isMobile ? '10px 8px' : '8px 10px',
            fontFamily: 'JetBrains Mono, monospace', fontSize: isMobile ? 9 : 10,
      ...
      {ANNOTATIONS.map(a => (
        <div key={a.num} style={{ border: '1.5px solid var(--ink)', background: 'var(--paper)', padding: isMobile ? '18px 16px' : '20px 22px', position: 'relative' }}>
          <div style={{
          ...
          <h3 style={{ fontFamily: 'Fraunces, serif', fontSize: isMobile ? 18 : 20, fontWeight: 500, color: 'var(--leaf-deep)', marginBottom: 8 }}
            dangerouslySetInnerHTML={{ __html: a.title.replace(/(\w+)$/, '<em style="font-style:italic; color:var(--clay)">$1</em>') }} />
          <p style={{ fontSize: isMobile ? 13 : 13.5, lineHeight: 1.6, color: 'var(--bark-soft)' }} dangerouslySetInnerHTML={{ __html: a.body }} />
          {a.items.length > 0 && (
            <ul style={{ listStyle: 'none', marginTop: 10 }}>
              {a.items.map((item, i) => (
                <li key={i} style={{ fontSize: isMobile ? 12 : 13, color: 'var(--bark-soft)', padding: '5px 0 5px 20px', position: 'relative', borderBottom: i < a.items.length - 1 ? '1px dashed var(--line-soft)' : 'none' }}>
  ...
  return (
    <div>
      ...
      {isMobile ? (
        /* Mobile: Vertical layout - Phone on top, annotations below */
